...
// and results. 
.factory('scanService', function ($http, $q, userService) {
    var obj = {};
    ...
    obj.getScans = function (token) {
        return $q(function (resolve, reject) {
            ...
            $http({
                ...
                if (!res.data) {
                    userService.getUser().then(function (userDetails) {
                        resolve(userDetails.scans);
                    }, function (error) {
                        reject();
                    })
                } else {
                    userService.saveScans(res.data);
                    resolve(res.data);
                }
            }, function (error) {
                userService.getUser().then(function (userDetails) {
...
.factory('userService', function (tokenService, $q) {
    ...
    obj.saveScans = function (scans) {
        window.localStorage.setItem("scans", scans);
    };
    return obj;
})
